...
describe("updating user / name", function () {
	...
	it("should fail when already exists", function (next) {
		var form = { name: userfix.user1.name, home: 'testname', email: 'testname@def.com', password: '1234' };
		express.put('/api/users/' + _user._id).send(form).end(function (err,res) {
			should(!res.error);
...
describe("updating user / email", function () {
	...
	it("should fail when already exists", function (next) {
		var form = { name: 'testemail', home: 'testmail', email: userfix.user1.email, password: '1234' };
		express.put('/api/users/' + _user._id).send(form).end(function (err,res) {
			should(!res.error);
