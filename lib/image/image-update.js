...
var error = require('../error/error');
	...
	app.put('/api/images/:id([0-9]+)', function (req, res) {
		usera.getUser(res, function (err, user) {
			...
			imagel.checkUpdatable(id, user, function (err) {
				if (err) return res.jsonErr(err);
				imagel.updateImage(id, form, function (err) {
					if (err) return res.jsonErr(err);
					res.json({});
				});
			});
	...
	exports.updateImageFields = function (id, fields, next) {
...
exports.checkUpdatable = function (id, user, next) {
	mongo.findImage(id, function (err, image) {
		...
		if (!image) {
			return next(error(error.ids.IMAGE_NOT_EXIST));
		}
...
exports.updateImage = function(id, form, _next) {
	...
	if (form.file) {
		checkImageMeta(form, function (err, meta) {
			if (err) return next(err);
			var dir = exports.getImageDir(id);
				...
				fs2.makeDirs(dir, function (err) {
					...
					fs.rename(form.file.tpath, org, function (err) {
						...
						exports.makeVersions(id, dir, org, meta.width, function (err, vers) {
							...
							mongo.updateImageFields(id, fields, next);
						});
					});
				});
			});
