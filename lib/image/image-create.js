...
function createOne(form, file, user, next) {
	exports.checkImageMeta(file, function (err, meta) {
		if (err) return next(err);
		...
		fs2.makeDirs(dir, function (err) {
			...
			fs.rename(file.tpath, org, function (err) {
				...
				site.makeVersions(org, meta, dir, id, function (err, vers) {
					...
					site.fillFields(image, form, vers);
...
exports.checkImageMeta = function (file, next) {
	imageb.identify([file.tpath], function (err, meta) {
		if (err) {
			return next(error(error.ids.IMAGE_TYPE))
		}
		site.checkImageMeta(meta, next);
	});
};
