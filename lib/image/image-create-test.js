...
describe("uploading small", function () {
	...
	it("should fail", function (next) {
		...
		express.post('/api/images').send(form).end(function (err, res) {
			...
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.PHOTO_SIZE));
			next();
		});
	});
});
...
describe("uploading text file", function () {
	...
	it("should fail", function (next) {
		...
		express.post('/api/images').send(form).end(function (err, res) {
			...
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.PHOTO_TYPE));
			next();
		});
	});
});
...
describe("uploading no file", function () {
	it("should fail", function (next) {
		...
		express.post('/api/images').send(form).end(function (err, res) {
			...
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.PHOTO_NO_FILE));
			next();
		});
	});
});
...
// 			should(res.body.err);
// 			should(error.find(res.body.err, error.ids.PHOTO_CYCLE));
// 			next();
// 		});
// 	});
// });
