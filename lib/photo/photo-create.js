...
init.add(function () {
	app.post('/api/photos', function (req, res) {
		usera.getUser(res, function (err, user) {
			...
			createPhoto(form, user, function (err, id) {
				if (err) return res.jsonErr(err);
				res.json({
					id: id
				});
	...
	app.get('/photos/new', function (req, res) {
		usera.getUser(res, function (err, user) {
			...
			var now = new Date();
			findHours(now, user, function (err, hours) {
...
createPhoto = function(form, user, _next) {
	var next = upload.deleter(form.files, _next);
	if (!form.file) {
		return next(error(ecode.PHOTO_NO_FILE));
	}
	exports.findHours(form.now, user, function (err, hours) {
		if (err) return next(err);
		if (hours > 0) {
...
exports.findHours = function(now, user, next) {
	var opt = {
		fields: { cdate: 1 },
		sort: { uid: 1, _id: -1 }
	}
	mongo.photos.findOne({ uid: user._id }, opt, function (err, photo) {
		if (err) return next(err);
		var hours = 0;
		if (photo) {
