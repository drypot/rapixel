...
init.add(function () {
	...
	app.post('/api/photos', function (req, res) {
		usera.getUser(res, function (err, user) {
			...
			photoc.createPhoto(form, user, function (err, id) {
				if (err) return res.jsonErr(err);
				res.json({
					ids: ids
				});
	...
	app.get('/photos/new', function (req, res) {
		usera.getUser(res, function (err, user) {
			...
			var now = new Date();
			photol.getTicketCount(now, user, function (err, free, hours) {
...
photoc.createPhotos = function(form, user, _next) {
	...
	function create() {
		...
		photoc.getTicketCount(form.now, user, function (err, free, hours) {
			if (err) return next(err);
			if (!free) {
				return next(null, ids);
			}
...
function createOne(form, file, user, next) {
	checkPhotoMeta(file, function (err, meta) {
		if (err) return next(err);
		var id = photoc.newPhotoId();
		var dir = photob.getPhotoDir(id);
		fs2.makeDirs(dir, function (err) {
			if (err) return next(err);
			var org = photoc.getOriginalPath(dir, id, meta.format);
...
photoc.checkPhotoMeta = function (form, next) {
	photoc.identify([form.file.tpath], function (err, meta) {
		if (err) {
...
photoc.makeVersions = function (id, dir, org, width, next) {
	...
	for (; i < _vers.length; i++) {
		...
		cmd	+= ' +repage'
		if (i == _vers.length - 1) {
			cmd += ' ' + photoc.getVersionPath(dir, id, dim.width);
		} else {
			cmd += ' -write ' + photoc.getVersionPath(dir, id, dim.width);
		}
	}
