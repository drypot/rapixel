...
before(function (next) {
	var users = [
		{ _id: userc.newId(), name: 'Name1', namel: 'name1', home: 'Home1', homel: 'home1', email: 'mail1@mail.com' }
	];
	mongo.users.insert(users, next);
});
...
describe("creating user / name check", function () {
	...
	it("check", function (next) {
		mongo.users.findOne({ email: 'nametest@mail.com' }, function (err, user) {
			should(!err);
...
describe("creating user / password check", function () {
	...
	it("check", function (next) {
		mongo.users.findOne({ email: 'passtest3847@mail.com' }, function (err, user) {
			should(!err);
