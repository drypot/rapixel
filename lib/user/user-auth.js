...
express.restoreLocalsUser = function (req, res, next) {
	if (req.session.uid) {
	...
	createSessionAuto(req, res, next);
};
...
function createSessionAuto(req, res, next) {
...
function validateUser(user, password, next) {
	if (!user) {
		return next(error(error.EMAIL_NOT_FOUND));
	}
	if (user.status == 'd') {
		return next(error(error.ACCOUNT_DEACTIVATED));
	}
	if (!userc.checkPassword(password, user.hash)) {
		return next(error(error.PASSWORD_WRONG));
	}
	next();
}
