...
var emailx = userc.emailx = /^[a-z0-9-_+.]+@[a-z0-9-]+(\.[a-z0-9-]+)+$/i

userc.getNewUserId = function () {
	return ++seed;
};

userc.makeHash = function (password) {
	return bcrypt.hashSync(password, 10);
}

userc.checkPassword = function (password, hash) {
	return bcrypt.compareSync(password, hash);
}

userc.getForm = function (body) {
	var form = {};
...
userc.createUser = function (form, next) {
	form.home = form.name;
	...
	userc.checkForm(form, 0, function (err) {
		...
		var now = new Date();
		var user = {
			_id: userc.getNewUserId(),
			name: form.name,
			namel: form.namel,
			home: form.home,
			homel: form.homel,
			email: form.email,
			hash: userc.makeHash(form.password),
			status: 'v',
			cdate: now,
			adate: now,
...
userc.checkForm = function (form, id, next) {
	var errors = [];
	var creating = id == 0;

	if (!form.name.length) {
		errors.push(error.ids.NAME_EMPTY);
	} else if (form.name.length > 32 || form.name.length < 2) {
		errors.push(error.ids.NAME_RANGE);
	}

	if (!form.home.length) {
		errors.push(error.ids.HOME_EMPTY);
	} else if (form.home.length > 32 || form.home.length < 2) {
		errors.push(error.ids.HOME_RANGE);
	}
	...
	countUsersByName(form.namel, id, function (err, cnt) {
		...
		countUsersByHome(form.homel, id, function (err, cnt) {
			if (err) return next(err);
			if (cnt) {
				errors.push(error.ids.HOME_DUPE);
			}
			countUsersByEmail(form.email, id, function (err, cnt) {
				if (err) return next(err);
				if (cnt) {
					errors.push(error.ids.EMAIL_DUPE);
				}
				if (errors.length) {
...
var checkFormEmail = userc.checkFormEmail = function (form, errors) {
	if (!form.email.length) {
		errors.push(error.ids.EMAIL_EMPTY);
	} else if (form.email.length > 64 || form.email.length < 8) {
		errors.push(error.ids.EMAIL_RANGE);
	} else if (!userc.emailx.test(form.email)) {
		errors.push(error.ids.EMAIL_PATTERN);
	}
}
...
var checkFormPassword = userc.checkFormPassword = function (form, errors) {
	if (!form.password.length) {
		...
		errors.push(error.ids.PASSWORD_RANGE);
	}
}
