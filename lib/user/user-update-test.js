...
var ucreate = require('../user/user-create');
var uupdate = require('../user/user-update');
...
describe("updating user / name", function () {
	...
	it("should fail when already exists", function (next) {
		var form = { name: userf.user1.name, home: 'testname', email: 'testname@def.com', password: '1234' };
		express.put('/api/users/' + _user._id).send(form).end(function (err,res) {
			should(!res.error);
