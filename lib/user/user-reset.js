			...
			mongo.resets.remove({ email: form.email }, function (err) {
				...
				mongo.resets.insert(reset, function (err, resets) {
					...
					var mail = {
						...
						text:
							'\n' +
							'Open the following URL to reset your ' + config.data.appName + ' password.\n\n' +
							config.data.frontUrl + '/users/reset-step2?id=' + reset._id + '&t=' + reset.token + '\n\n' +
							config.data.appName
					};
