...
init.add(function () {
	...
	app.get('/api/users/:id([0-9]+)', function (req, res) {
		...
		userv.getCached(id, function (err, _tuser) {
	...
	app.get('/users/:id([0-9]+)', function (req, res) {
		var id = parseInt(req.params.id) || 0;
		userv.renderProfile(req, res, id);
	});
...
userv.getCached = function (id, next) {
	var user = users[id];
	if (user) {
...
userv.getCachedByHome = function (homel, next) {
...
userv.findAndCache = function (email, next) {
