...
describe("login", function () {
	...
	it("should fail with invalid email", function (next) {
		...
		express.post('/api/sessions').send(form).end(function (err, res) {
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.EMAIL_NOT_FOUND));
			next();
		})
	});
	it("should fail with invalid password", function (next) {
		...
		express.post('/api/sessions').send(form).end(function (err, res) {
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.PASSWORD_WRONG));
			next();
		})
	});
});
...
describe("accessing user resource", function () {
	...
	it("should fail", function (next) {
		express.get('/test/user').end(function (err, res) {
			...
			should(!res.error);
			should(res.body.err);
			should(error.find(res.body.err, error.ids.NOT_AUTHENTICATED))
			next();
		})
	});
});
...
describe("accessing user resouce ", function () {
	...
	describe("with auto login with invalid email", function () {
		...
		it("given email changed", function (next) {
			...
			mongo.users.update({ _id: userf.user1._id }, fields, function (err, cnt) {
				should(!err);
				should(cnt);
				next();
			});
