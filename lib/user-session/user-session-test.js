...
var userss = require('../user-session/user-session');
var userfix = require('../user-session/user-fixture');
...
init.add(function () {
	...
	app.get('/test/admin', function (req, res) {
		userss.getAdmin(res, function (err, user) {
			if (err) return express.jsonErr(res, err);
			res.json({});
		});
	});
...
describe("login", function () {
	it("should success", function (next) {
		...
		express.post('/api/sessions').send(form).end(function (err, res) {
			...
			should(!res.error);
			should(!res.body.err);
			res.body.user.id.should.equal(userfix.user1._id);
			res.body.user.name.should.equal(userfix.user1.name);
			next();
		})
	});
...
describe("accessing user resouce ", function () {
	...
	describe("with auto login", function () {
		...
		it("given user session with auto login", function (next) {
			userfix.loginUser1WithRemember(next);
		});
		it("should success", function (next) {
			express.get('/test/user').end(function (err, res) {
				should(!err);
	...
	describe("with auto login with invalid email", function () {
		...
		it("checking email cookie", function (next) {
			express.get('/test/cookies').end(function (err, res) {
				...
				should(!res.error);
				should(!res.body.err);
				res.body.email.should.equal(userfix.user1.email);
				next();
			});
		});
		it("given email changed", function (next) {
			...
			mongo.users.update({ _id: userfix.user1._id }, fields, function (err, cnt) {
				should(!err);
				should(cnt);
				next();
			});
