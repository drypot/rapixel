...
function shouldExists(path) {
	fs.existsSync(path).should.be.true;
}

function shouldNotExists(path) {
...
describe("deleter", function () {
	...
	it("can be confirmed", function (next) {
		shouldNotExists(_path1);
		shouldNotExists(_path2);
		shouldExists(_path3);
		next();
	});
});
...
describe("uploading one file", function () {
	it("should success", function (next) {
		...
		express.post('/api/upload').attach('files', f1).end(function (err, res) {
			...
			shouldExists(upload.getPath(file.tname));
			next();
		});
	});
});
...
describe.only("deleting files", function () {
	var _files;
